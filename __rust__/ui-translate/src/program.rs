...
pub async fn program(
    ...
    let mut exist_words = equals(v1, v2);

    // test block, mb slowly
    for i in 0..exist_words.len() {
        if exist_words[i].contains("value") {
            exist_words.remove(i);
        }
    }
    // test block, mb slowly

    *max = exist_words.len() as f32;
...
fn reader(path: &mut String) -> Result<(), Box<dyn std::error::Error>> {
    // if !Path::new(path.trim()).exists() {
    //     panic!("!!!\nFile not found -- {} --\n!!!", path.trim());
    //     // Ok(())
    // }

    let mut reader_var = OpenOptions::new().read(true).open(path.trim())?;
    path.clear();
    reader_var.read_to_string(path)?;
    Ok(())
}
...
fn write_docx(texts: &Vec<String>, output: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    //     Err(panic!(
    //         "Не могу найти папку: {FILENAME}. Попробуйте создать её вручную возле .exe файла."
    //     ));
    // }
